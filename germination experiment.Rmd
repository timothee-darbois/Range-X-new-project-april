...
df_germination<-df_germination %>% mutate(condi=paste(df_germination$site,df_germination$plotID,sep=" "))

df_germination<-df_germination %>% mutate(positionID_clean=str_sub(df_germination$positionID,,2)) #withdraw the number of the replicate that is written in the same column
df_germination<-df_germination %>% mutate(nb_replicate=sub("^$","1",gsub("\\D+", "",str_sub(df_germination$positionID,3,5)))) #add a column with the replicate ID ( 1 to 3) 


df_germination<-df_germination %>% mutate(merge_ID=paste(df_germination$site,df_germination$blockID,df_germination$plotID,df_germination$positionID_clean, sep=" "))
...
```{r}
df_percentage<-df_percentage %>% mutate(total_germinated= rowSums(df_percentage[c("Day.0","Day.2","Day.4","Day.6","Day.8","Day.12","Day.16","Day.24","Day.32")],na.rm=TRUE))

df_percentage<-df_percentage %>% mutate(germination_percentage=100*df_percentage$total_germinated/df_percentage$Number.of.seeds.for.germination)

df_percentage<-df_percentage %>% mutate(germinated_and_viable=rowSums(df_percentage[c("total_germinated","Viable.seeds")],na.rm=TRUE)) ## only 1 of 3 individual were tested: if no test, don't make the sum
...
for (i in row.names(df_percentage)){
  my_t50<-c(my_t50,df_percentage[i,c("Day.0","Day.2","Day.4","Day.6","Day.8","Day.12","Day.16","Day.24","Day.32")] %>%
                 replace(is.na(.),0) %>%
                 as.numeric() %>% 
                 t50(intervals=c(0,2,4,6,8,12,16,24,32),method=c("coolbear"))) #calcule T50 : when there is missing value in the table, mean =0 
}
...
plot_model(glmm_gp_tc, show.values = TRUE, width = 0.1, title = "interactions of temperature and competition on germination percentage") 
```
### post hoc analysis
```{r}
...
glmm_t501<- lmer(t50~condi + (1|blockID)+ (1|species), data=df_percentage)
glmm_t501_log<- lmer(t50_log~condi + (1|blockID)+ (1|species), data=df_percentage)


glmm_t502<- lmer(t50~site + (1|blockID)+ (1|species), data=df_percentage)

glmm_t503<- lmer(t50~temperature +  (1|blockID)+ (1|species), data=df_percentage)


glmm_t504<- lmer(t50~competition +  (1|blockID)+ (1|species), data=df_percentage)
...
glmm_t50_tc<- lmer(t50~temperature*competition + (1|blockID)+ (1|species), data=df_percentage)

glmm_t50_log_tc<- lmer(t50_log~temperature*competition + (1|blockID)+ (1|species), data=df_percentage)


sjPlot::tab_model(glmm_t50_tc,show.intercept = T)
plot_model(glmm_t50_tc, show.values = TRUE, width = 0.1, title = "interactions of temperature and competition on t50") 
```
### post hoc analysis
```{r}
