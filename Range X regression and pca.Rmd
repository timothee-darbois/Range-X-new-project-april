...
```
data = demo_trait, aes(x = height_veg_cm, y = height_rep_cm, 
```{r}
glmm_vegh_reph<-lmer(height_veg_cm~height_rep_cm*competition+(1|species)+(1|block),data=demo_trait)
interact_plot(glmm_vegh_reph,pred=height_rep_cm,modx=competition,interval =TRUE, int.type="confidence")
```
...
```{r}
glmm_vegh_reph<-lmer(weight.seed~height_veg_cm*site+(1|species)+(1|block),data=all_trait_pca)
interact_plot(glmm_vegh_reph,pred=height_veg_cm,modx=site,interval =TRUE, int.type="confidence")
```
...
```{r}
glmm_vegh_reph<-lmer(weight.seed~no_flowers_tt*treat2.y+(1|species)+(1|block),data=all_trait_pca)
interact_plot(glmm_vegh_reph,pred=no_flowers_tt,modx=treat2.y,interval =TRUE, int.type="confidence")
```
