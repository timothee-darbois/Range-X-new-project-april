...
### check performance
```{r}
check_glmm_test<-check_model(gllm_test)
...
```
seems not so bad in term of overdispersion ?
final graph
```{r}
a<-fixef(glmm_sn1)
b<-as.data.frame(a)
...
NS
```{r}
glmm_sn_tc<- glmer(seeds_number.inf~temp*treat2 + (1|blockID)+ (1|species), data=df,family=poisson)
...
```{r}
summary(glht(glmm_sn_LM, mcp(condi="Tukey")))
...
plot_finalw<-ggplot(df_plotw,aes(x=competition,y=value,fill=temperature))+
  ...
  geom_path(aes(group = temperature, color = temperature))+
  ylab("value of glmm")+
  ggtitle("Only warming has an effect on seed weight ")
plot_finalw
